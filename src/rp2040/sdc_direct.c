...
void sdc_boot(void) {
  ...
  // try to open core.fs
  FIL fil;
  if(f_open(&fil, "core.fs", FA_OPEN_EXISTING | FA_READ) == FR_OK) {
    sdc_debugf("core.fs found");
	...
	jtag_gowin_writeSRAM_postproc(0);
      } else
	fatal_debugf("bitstream header parsing failed");
      
      if(fr != FR_OK)
	fatal_debugf("Read error after %lu bytes: %d", total, fr);
      else
    ...
    // reconfig the FPGA and allow it to boot from flash
    mcu_hw_fpga_reconfig(true);
  }
